...
// word limit. Without a target profile the gate reports nothing at all - a manuscript with no
// venue has no venue rules to break.
//
...
export const profileGate = {
  ...
  run(text, ctx) {
    ...
    return checkSection(profile, {
      section: ctx.section,
      sectionOrder: ctx.sectionOrder,
      text,
    }).map(({ severity, message, hint }) => ({ gate: NAME, severity, line: 1, message, hint }));
  },
};
