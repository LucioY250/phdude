...
// faithful, reproducible sample rather than hand-authored YAML. The mechanics live in
// lib/example-builder.mjs; what is below is this example's research and nothing else.
...
const ROOT = join(EXAMPLES_DIR, 'generic-thesis');
...
// search old enough that `phdude gaps` and `phdude next` both call it stale. The preprint is
// deliberately left pending: the policy requires approval for one.
function recordStaleSearch(deps, questionId) {
  return recordSearch(deps, {
    query: 'note-taking app adoption undergraduates',
    question: questionId,
    at: '2025-06-01T00:00:00Z',
    provider: 'openalex',
    routes: [{ match: 'api.openalex.org', body: OPENALEX_RESPONSE }],
  });
}
...
function recordMethodologyReview(deps, method, evidence) {
  return recordReview(deps, 'methodology', [
    {
      target: method.id,
      severity: 'major',
      message:
...
async function writeIntroduction(deps, claim) {
  ...
  await submitDraft(deps, 'introduction', draftIntroduction(claim));
  await submitDraft(deps, 'introduction', revisedIntroduction(claim), { revision: true });

  await prose.proseSection(deps, 'introduction');

  await approveSection(deps, {
...
export const genericThesis = {
  name: 'generic-thesis',
  title: 'Generic Thesis Example',
  sources: SOURCE_FILES,
  async build(deps, { root, artifactId }) {
    const artAlpha = artifactId('sources/survey-alpha.md');
    const artBeta = artifactId('sources/survey-beta.md');
    const artGamma = artifactId('sources/survey-gamma.md');
    const artCsv = artifactId('sources/participants.csv');
    const artNotes = artifactId('sources/lab-notebook.md');

    // Classifying an artifact is bootstrap's first step, and it is what separates an
    // unclassified file from one that is classified but never mined (`phdude gaps`'
    // `artifact-unmined`).
    ...
    await acceptCandidate(deps, await recordStaleSearch(deps, rq.id));

    await addAuthorProfile(deps, root, RESEARCHER_A_PROFILE, RESEARCHER_A_SAMPLE);
    ...
    await analyseTheSurvey(withAnalysis(deps, root), claimChannel);
    ...
    // The review last, because reviewing is what happens once there is something to review.
    await recordMethodologyReview(deps, method, [evidence1, evidenceBeta]);
  },
};

export async function generate(root) {
  await buildExample(root, genericThesis);
}
